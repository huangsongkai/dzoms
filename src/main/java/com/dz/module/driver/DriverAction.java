...
public class DriverAction extends BaseAction{
	...
	public String driverCheck(){
		...
		for(Driver driver:drivers){
			if(driver.getStatus() == 0 && isLuRuRen){
				urlAttachBeen.add(new UrlAttachBean(driver.getIdNum(),driver.getName(),"/DZOMS/driver/driverPreupdate?driver.idNum="+driver.getIdNum()+"&hzw=123"));
			}
			if(driver.getStatus() == 1 && isYunYinBuJinLi){
				urlAttachBeen.add(new UrlAttachBean(driver.getIdNum(),driver.getName(),"/DZOMS/driver/driverToDifferentPage?driver.idNum="+driver.getIdNum()+"&hzw=1"));
			}
			if(driver.getStatus() == 2 && isCaiWuJinLi){
				urlAttachBeen.add(new UrlAttachBean(driver.getIdNum(),driver.getName(),"/DZOMS/driver/driverToDifferentPage?driver.idNum="+driver.getIdNum()+"&hzw=2"));
			}
		}
	...
	public String driverAdd() throws IOException {
		...
		if(bankCard!=null){
			...
			bankCard.setIsDefaultPay(true);
			bankCard.setIsDefaultRecive(true);
	...
	public String driverToExcel(){
		try(InputStream is = new FileInputStream(System.getProperty("com.dz.root")+"driver/driver_template.xls")) {
			...
			try (OutputStream os = new FileOutputStream(file)) {
	            ...
	            int len = driverService.driverSearchTotal();
	            Page page = PageUtil.createPage(len,len, 0);
	            List<Driver> l = driverService.driverSearch(page);
	            
	            context.putVar("drivers", l);
