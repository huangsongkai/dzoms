...
<head lang="en">
	...
	<script type="text/javascript">
		...
		function dateRefresh() {
			var arr = $("#startdate").val().split("/");
			//var arr = $("#operate_card_date").val().split("/");
...
//    });
		...
		$(document).ready(function(){
			if ($('[name="contract.contractBeginDate"]').val().trim().length==0) {
				...
				$('[name="contract.contractBeginDate"]').val(noe_date.format("yyyy/MM/dd"));
			}
...
<div class="padding">
	<form method="post" action="/DZOMS/contract/contractWrite" class="form-inline" style="width: 100%">
		...
		%>
		<s:set name="approval" value="%{@com.dz.common.other.ObjectAccess@execute('from VehicleApproval where contractId='+#request.quate+contract.id+#request.quate)}"></s:set>
				...
				<div class="line">
					...
					<div class="xm10">
						...
						<div class="form-group">
							...
							<div class="field" >
								<%--<s:textfield cssClass="input datepick"  id="startdate_raw" onBlur="set_date()" name="contract.contractBeginDate" />
						...
						<div class="form-group">
							...
							<div class="field" >
								<s:textfield cssClass="input datepick" id="enddate" onchange="javascript:dateRefresh();" onBlur="dateChanged()" name="contract.contractEndDate" />
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="field" >
								<s:textfield cssClass="input" name="totalRent" value="0" />
							</div>
						</div>
						<div class="form-group">
							<div class="label padding">
							...
							<div class="field" >
								<s:textfield cssClass="input" name="contract.rentFirst" value="0" />
							</div>
						</div>
						<div class="form-group">
							<div class="label padding">
							...
							<div class="field" >
								<s:textfield cssClass="input" name="rentFirst_Month" value="96" />
						...
						<div class="form-group">
							...
							<div class="field" style="width:160px;">
								<s:select cssClass="input" name="rentType" value="" list="{'平均租金','自定义年租金'}" onchange="javascript:rentTypeChange();" />
							</div>
						</div>
						<div class="form-group" style="margin:5px 0">
							...
							<div class="field" >
								<a class="button bg-green button-big genButton" href="javascript:void(0);" onclick="genRentMonthPlan()">生成租金计划</a>
							</div>
						</div>
						<br>
						<div id="rentPerYear">
						...
						<script>
							function  rentTypeChange() {
								if($("#rentType").val() == "自定义年租金"){
									var rentYearInputs = "";
									var template = '<div class="form-group"><div class="label padding"><label>第$num年租金</label></div>'+
											'<div class="field"><s:textfield cssClass="input" value="0"/></div></div>';
									var begY = $("#startdate").val().substring(0,$("#startdate").val().indexOf("/"));
									var endY = $("#enddate").val().substring(0,$("#enddate").val().indexOf("/"));
									var yearSpan = endY - begY;
									if(yearSpan > 8)
										yearSpan = 8;
									for(var i = 0; i < yearSpan; i++){
										rentYearInputs += template.replace("$num",i+1);
									}
									$("#rentPerYear").html(rentYearInputs);
								}else{
									$("#rentPerYear").html("");
								}
								$('.genButton').removeAttr("disabled");
							...
							function genRentMonthPlan() {
								$(".genButton").attr("disabled",true);
								var begY = $("#startdate").val().substring(0,4);
								var begM = parseInt($("#startdate").val().substring(5,7));
								var endY = $("#enddate").val().substring(0,4);
								var endM = parseInt($("#enddate").val().substring(5,7));
								var months = (endY-begY)*12+endM-begM;
								if(months>96){
									months = 96;
								}
								var firstRent;
								var avgRent;
								//为了避免余数,平均的话处理好第一个月,每年不一样的处理好每年第一个月.
								if($("#rentType").val() == "平均租金"){
									avgRent = parseInt(($("#totalRent").val()-$("#contract_rentFirst").val())/months);
									firstRent = $("#totalRent").val()-$("#contract_rentFirst").val() - avgRent*(months-1);
								}
								var indexY = begY;
								var $tr = $('<tr class="hide data-tr"></tr>').append($("<td />").text((indexY)+"年"));
								$("#plan tr.data-tr").remove();
								var $plan = $("#plan");
								for(var m = 1; m < begM; m++){
									$td = $("<td />").text("  ").appendTo($tr);
								}
								for(var m = 0; m < months; m++){
									if((m+begM)%12 == 1){//如果是年初 增加新行
										$plan.append($tr);
										$tr = $('<tr class="hide data-tr"></tr>').append($("<td />").text((++indexY)+"年"));
									}
									var $td = $("<td />").appendTo($tr);
									//处理首年
									if(m%12 == 0 && $("#rentType").val() != "平均租金"){
										var divideMonth = months - m < 12? months -m : 12;
										avgRent = parseInt($($("#rentPerYear input")[indexY-begY]).val()/divideMonth);
										firstRent = $($("#rentPerYear input")[indexY-begY]).val() - avgRent*(divideMonth-1);
									}

									if(m == 0 && $("#rentType").val() == "平均租金"){
										var $input = $('<input class="data-td input auto" size="10" />').val(firstRent).appendTo($td);
										rentArr.push(firstRent);
										firstRent = avgRent;
										continue;
									}
									if(m%12 ==0){
										$('<input class="data-td input auto" size="10" />').val(firstRent).appendTo($td);
										rentArr.push(firstRent);
									}else{
										$('<input class="data-td input auto" size="10" />').val(avgRent).appendTo($td);
										rentArr.push(avgRent);
									}
								...
								geneRentPlanState=1;
								var planListArr = [];
								planListArr.push({"begin":$("#startdate").val(),"end":$("#enddate").val(),"money":$("#totalRent").val(),"comment":"","rentType":$("#rentType").val()});
								$("input[name='planList']").val($.toJSON(planListArr));
							}
						...
						<div class="form-group">
							...
							<div class="field" >
								<s:if test="%{contract.contractFrom!=null}">
									<input value="是" readonly="readonly">
								</s:if>
								<s:else>
						...
						<div class="form-group">
							...
							<div class="field">
								<textarea type="text" id="licenseNum" name="vehicle.licenseNum" class="input" disabled="disabled"></textarea>
							</div>
						</div>
					</div>
				</div>
...
//	if(hasNotFilled>=0){
//		var nyear = beginMonth.year * 1 + (hasNotFilled/12-0.5).toFixed(0)*1;
//		var nmonth = hasNotFilled - (hasNotFilled/12-0.5).toFixed(0)*12 + beginMonth.month*1;
//		if(nmonth>12){
//			nyear++;
//			nmonth-=12;
//		}
...
//	}else
		{
			...
			if($("input[name='contract.rentFirst']").val().trim().length==0){
				$("input[name='contract.rentFirst']").val("0");
			}
			//预付租金摊销
