...
//SimpleCanleder_Year_Month 仅用于年、月选取的替代
(function($) {
	$.simpleCanleder = function(box, options){
		...
		var begin_month = options.begin_month;
		...
		function _buildBody(canlederBox){
			...
			for(var i = 0; i < 12; i++){
				...
				var $li = $("<li/>").addClass("my-simple-calender-month").addClass("my-simple-calender-month"+i).append($inner).click(function(){
					var year = canlederBox.find(_title_ul_li).eq(1).find("select").val();
					var month = $(this).find("div.inner").text() * 1;
					
					if(year==begin_year){
						if(begin_month!=undefined&&month<begin_month) return;
					}
						
					if(year==end_year){
						if(end_month!=undefined&&month>end_month) return;
					}
		...
		function _getSelect(year){
			...
			$select = $("<select/>");
			if(begin_year==undefined||end_year==undefined){
				for(var i = 10; i >=0; i--){
				...
				/*if(begin_month!=undefined&&end_month!=undefined){
					$select.change(function(){
						var freshYear = $select.val();
						
						$(".my-simple-calender-month").removeClass("bg-gray");
						$(".my-simple-calender-month").addClass("bg-blue-light");
						if(freshYear==begin_year){
							for(var i=0;i<begin_month-1;i++){
								$(".my-simple-calender-month"+i).removeClass("bg-blue-light");
								$(".my-simple-calender-month"+i).addClass("bg-gray");
							}
						}
						if(freshYear==end_year){
							for(var i=end_month-1;i<12;i++){
								$(".my-simple-calender-month"+i).removeClass("bg-blue-light");
								$(".my-simple-calender-month"+i).addClass("bg-gray");
							}
						}
