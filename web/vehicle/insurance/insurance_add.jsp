...
//  	alert(actionName);
    	...
    	$("#insuranceClass").change(function(){
    		if ($("#insuranceClass option:eq(0)").is(":selected")) {
    			...
    			$("#insuranceNum").attr("data-validate","required:请选择,regexp#(^PDAA[A-Z0-9]{18}$):格式不正确(22位，交强险以PDZA开头，商险以PDAA开头，承运人责任险以PDZS开头)");
    		} else if ($("#insuranceClass option:eq(1)").is(":selected")){
    			$("#insuranceNum").val('<%=jqx%>');
    			$("#insuranceNum").attr("data-validate","required:请选择,regexp#(^PDZA[A-Z0-9]{18}$):格式不正确(22位，交强险以PDZA开头，商险以PDAA开头，承运人责任险以PDZS开头)");
    			itemsDefault("#insuranceMoney","insuranceMoneyJq");
    		}else{
    			$("#insuranceNum").val('<%=cyrx%>');
    			$("#insuranceNum").attr("data-validate","required:请选择,regexp#(^PDZS[A-Z0-9]{18}$):格式不正确(22位，交强险以PDZA开头，商险以PDAA开头，承运人责任险以PDZS开头)");
...
<div class="adminmin-bread" style="width: 100%;">
    ...
    <div class="xm7 xm2-move">
    	...
    	<div class="form-group">
            ...
            <div class="field">
            	<s:if test="%{bean==null||bean[0]==null}">
            		<s:select cssClass="input" id="insuranceClass" name="insurance.insuranceClass" list="#{'商业保险单':'商业保险单','交强险':'交强险','承运人责任险':'承运人责任险'}" value="交强险" ></s:select>
            	</s:if>
            	<s:else>
        ...
        <div class="form-group">
            ...
            <div class="field form-inline">
                <s:textfield  cssClass="input" name="insurance.insuranceCompany" id="insuranceCompany" value="%{bean[0].insuranceCompany}"  data-validate="required:请选择"/>
 ...
 <td class="delete selected_able"><a href="javascript:deleteV('<s:property value="%{#v.id}"/>')">删除</a></td>
	...
	<div class="xm9-move xm2">
		<form action="/DZOMS/vehicle/insurance_relook" method="post">
			<input type="hidden" name="url" value="/vehicle/insurance/insurance_add.jsp"/>
